...
export function validateGptImage2Size(width: number, height: number): SizeValidation {
    if (!Number.isFinite(width) || !Number.isFinite(height) || width <= 0 || height <= 0) {
        return { valid: false, reason: '宽度和高度必须为正数。' };
    }
    if (!Number.isInteger(width) || !Number.isInteger(height)) {
        return { valid: false, reason: '宽度和高度必须为整数。' };
    }
    ...
    if (width > GPT_IMAGE_2_MAX_EDGE || height > GPT_IMAGE_2_MAX_EDGE) {
        return { valid: false, reason: `单边最大为 ${GPT_IMAGE_2_MAX_EDGE}px。` };
    }
    ...
    if (pixels < GPT_IMAGE_2_MIN_PIXELS) {
        return { valid: false, reason: `总像素必须不少于 ${GPT_IMAGE_2_MIN_PIXELS.toLocaleString()}。` };
    }
    if (pixels > GPT_IMAGE_2_MAX_PIXELS) {
        return { valid: false, reason: `总像素必须不超过 ${GPT_IMAGE_2_MAX_PIXELS.toLocaleString()}。` };
    }
    return { valid: true };
}
